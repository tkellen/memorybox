...
func TestNewFromConfig(t *testing.T) {
	expected := "test"
	actual := store.NewLocalDiskStoreFromConfig(map[string]string{
		"home": expected,
	})
...
func TestLocalDiskStore_String(t *testing.T) {
	store := store.NewLocalDiskStore("/")
	actual := store.String()
...
func TestLocalDiskStore_Put(t *testing.T) {
	tempDir, tempErr := ioutil.TempDir("", "*")
	if tempErr != nil {
		t.Fatalf("test setup: %s", tempErr)
	}
	defer os.RemoveAll(tempDir)
	store := store.NewLocalDiskStore(tempDir)
	filename := "test"
...
func TestLocalDiskStore_Put_CannotCreateHome(t *testing.T) {
	file, tempErr := ioutil.TempFile("", "*")
	if tempErr != nil {
		t.Fatalf("failed setting up test: %s", tempErr)
	}
	defer os.Remove(file.Name())
	store := store.NewLocalDiskStore(file.Name())
	filename := "test"
	expected := []byte(filename)
	putErr := store.Put(bytes.NewReader(expected), filename)
	if putErr == nil {
...
func TestLocalDiskStore_Put_BadReader(t *testing.T) {
	tempDir, tempErr := ioutil.TempDir("", "*")
	if tempErr != nil {
		t.Fatalf("test setup: %s", tempErr)
	}
...
func TestLocalDiskStore_Put_CannotCreateFile(t *testing.T) {
	tempDir, tempErr := ioutil.TempDir("", "*")
	if tempErr != nil {
		t.Fatalf("test setup: %s", tempErr)
	}
...
func TestLocalDiskStore_Get(t *testing.T) {
	tempDir, tempErr := ioutil.TempDir("", "*")
	if tempErr != nil {
		t.Fatalf("test setup: %s", tempErr)
	}
...
func TestLocalDiskStore_Exists(t *testing.T) {
	tempDir, tempErr := ioutil.TempDir("", "*")
	if tempErr != nil {
		t.Fatalf("test setup: %s", tempErr)
	}
...
func TestLocalDiskStore_Search(t *testing.T) {
	tempDir, tempErr := ioutil.TempDir("", "*")
	if tempErr != nil {
		t.Fatalf("test setup: %s", tempErr)
	}
