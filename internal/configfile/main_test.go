...
func TestConfigNew(t *testing.T) {
	...
	for name, test := range table {
		t.Run(name, func(t *testing.T) {
			_, err := configfile.NewConfigFile(test.input)
...
func TestConfigFile_String(t *testing.T) {
	expected := "targets:\n  test:\n    home: ~/memorybox\n    type: localDisk\n"
	actual := &configfile.ConfigFile{
		Targets: map[string]configfile.Target{
			"test": {
				"type": "localDisk",
				"home": "~/memorybox",
			},
...
func TestTarget_Get(t *testing.T) {
	expected := "value"
	target := &configfile.Target{"key": expected}
...
func TestTarget_Delete(t *testing.T) {
	target := (&configfile.Target{"key": "value"}).Delete("key")
